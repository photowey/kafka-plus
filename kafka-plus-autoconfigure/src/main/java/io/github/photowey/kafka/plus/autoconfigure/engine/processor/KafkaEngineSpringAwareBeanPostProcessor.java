...
public class KafkaEngineSpringAwareBeanPostProcessor extends AbstractEngineAwareBeanPostProcessor<KafkaEngine, KafkaEngineSpringAware> {
    ...
    public void inject(ConfigurableListableBeanFactory beanFactory, Object bean) {
        if (bean instanceof KafkaEngineSpringAware) {
            KafkaEngine engine = this.beanFactory.getBean(KafkaEngine.class);
            ((KafkaEngineSpringAware) bean).setKafkaEngine(engine);
        }
    }
}